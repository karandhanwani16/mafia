...
const Voting = () => {
  ...
  const { players, votes, voteCount, phase } = useSelector((state) => state.game);
  const { currentPlayer } = useSelector((state) => state.player);
  const [selectedTarget, setSelectedTarget] = useState(null);
  const [voted, setVoted] = useState(false);
  const socket = getSocket();

  const meInGame = players.find(p => String(p.playerId) === String(currentPlayer?.playerId));
  const isAlive = meInGame?.isAlive !== false;
  const alivePlayers = players.filter(p => p.isAlive && String(p.playerId) !== String(currentPlayer?.playerId));
  const hasVoted = votes.some(v => String(v.voterId) === String(currentPlayer?.playerId));
      ...
      {hasVoted ? (
        <div className="text-center py-4 animate-success-pop">
          ...
          <p className="text-mafia-muted text-sm mb-4">
            {phase === 'results'
              ? 'Vote results are in. Next round starting soon…'
              : votesRemaining > 0
                ? `Waiting for ${votesRemaining} more vote(s) from alive players…`
